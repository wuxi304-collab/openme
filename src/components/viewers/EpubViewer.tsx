...
export default function EpubViewer({ filePath }: Props) {
  ...
  useEffect(() => { let disposed = false; window.electronAPI.readEpub(filePath).then((result) => { if (disposed) return; if (result.success && result.book) setBook(result.book); else setError(result.message ?? "EPUB 无法读取"); }).catch((reason) => { if (!disposed) setError(reason instanceof Error ? reason.message : "EPUB 无法读取"); }); return () => { disposed = true; }; }, [filePath]);
  ...
  if (error) return <div className="viewer-error" role="alert"><strong>EPUB 无法预览</strong><p>{error}</p></div>;
  ...
  return (
    <div className="epub-viewer">
      ...
      <main className="epub-reader"><div className="epub-toolbar"><span>{chapter + 1} / {book.chapters.length}</span><label><span className="sr-only">搜索本章</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索本章…" /></label><div className="viewer-tools"><button type="button" aria-label="减小字号" onClick={() => setFontSize((value) => Math.max(13, value - 1))}>字−</button><button type="button" aria-label="增大字号" onClick={() => setFontSize((value) => Math.min(28, value + 1))}>字＋</button></div></div><article style={{ fontSize }}><h1>{book.chapters[chapter].title}</h1>{paragraphs.map((paragraph, index) => !needle || paragraph.toLocaleLowerCase().includes(needle) ? <p key={index}>{paragraph}</p> : null)}</article><footer><button type="button" disabled={chapter === 0} onClick={() => setChapter((value) => value - 1)}>上一章</button><button type="button" disabled={chapter === book.chapters.length - 1} onClick={() => setChapter((value) => value + 1)}>下一章</button></footer></main>
    </div>
  );
}
