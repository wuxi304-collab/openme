    ...
    <header className="app-titlebar">
      ...
      <div className="window-controls no-drag">
        <label style={{display:'inline-flex',alignItems:'center',gap:6,marginRight:8}}>
          <select aria-label={t('language')} value={lang} onChange={(e) => setLang(e.target.value === 'en' ? 'en' : 'zh')} style={{height:28,borderRadius:6}}>
            <option value="zh">{t('chinese')}</option>
            <option value="en">{t('english')}</option>
          </select>
        </label>
        <button type="button" title={theme === 'dark' ? t('light') : t('dark')} aria-label={theme === 'dark' ? t('light') : t('dark')} onClick={() => toggle()} style={{marginRight:8}}>{theme === 'dark' ? '☀' : '🌙'}</button>
