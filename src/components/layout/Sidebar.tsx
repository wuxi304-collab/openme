...
interface Props { files: FileInfo[]; selectedPath: string | null; onSelect: (file: FileInfo) => void; onRemove: (file: FileInfo) => void; onOpenDialog?: () => void; searchValue?: string; onSearchChange?: (value: string) => void; }

export default function Sidebar({ files, selectedPath, onSelect, onRemove, onOpenDialog, searchValue, onSearchChange }: Props) {
  return (
    <aside className="workspace-sidebar">
      <div className="sidebar-actions"><button type="button" onClick={onOpenDialog} className="open-file-button"><span className="button-brick" aria-hidden="true">+</span><span>打开文件</span><kbd>Ctrl O</kbd></button><label className="file-search"><svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="7"/><path d="m16.2 16.2 4 4"/></svg><span className="sr-only">搜索最近文件</span><input value={searchValue} onChange={(event) => onSearchChange?.(event.target.value)} placeholder="搜索最近文件…" autoComplete="off" /></label></div>
